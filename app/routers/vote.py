...
def vote(vote: schemas.Vote, db: Session = Depends(database.get_db), 
         current_user: schemas.TokenData = Depends(oauth2.get_current_user)) -> str:

    post_id = db.query(models.Post).filter(models.Post.id == vote.post_id)
    ...
    if (vote.dir == 1):
        ...
        db.add(new_vote)
        db.commit()
        return f"Successfuly added vote"
    else:
        if not found_vote:
